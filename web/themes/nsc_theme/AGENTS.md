# NscTheme Theme - Agent Rules

This document contains coding rules and conventions for the NscTheme theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in NscTheme theme components.
...
{{
  include(
    '@nsc_theme/components/icon/icon.twig',
    {
      weight: 'bold',
      icon: icon
    }
...
```twig
{% include '@nsc_theme/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the NscTheme theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the NscTheme theme directory (`web/themes/custom/nsc_theme/`). Running these commands ensures that:
